...
searchMaxvision <- function(dateBegin="1/1/2016", dateEnd="2/1/2016", 
                            fileName="", fileType="ALL", step="ALL") {
    ...
    parseHTML <- function(htmlCode) {
        ...
        results <- as.data.frame(matrix(results, ncol=14, byrow=TRUE))
    ...
    # temp initial values for debugging
    #dateBegin <- "1/1/2013"
    #dateEnd <- "4/1/2013"
    #fileName <- ""
    #fileType <- "SPD"
    ...
    dateBegin <- as.Date(mdy(dateBegin))
    dateEnd <- as.Date(mdy(dateEnd))
    ...
    print(paste("Fetching results for date range: ", dateBegin2, 
                " to ", dateEnd2, sep =""))
    
    
    # Fetch html source code
    theurl <- constructURL(begin = dateBegin2, end = dateEnd2, fname = fileName, 
                           type = fileType, step = step, page = 1)
    ...
    # append results to my_results data frame
    if(nrow(results) > 0) {
        if(nrow(my_results) == 0) {
            my_results <- results
        } else {
    ...
    if(any(str_detect(webpage, "Showing"))) {
        ...
        while(latestFile != lastFile) {
            ...
            # fetch html source code
            theurl <- constructURL(begin = dateBegin2, end = dateEnd2, fname = fileName, 
                                   type = fileType, step = step, page = pageNum)
            webpage <- readLines(tc <- textConnection(getURL(theurl))); close(tc);
            rm(tc, theurl)
...
scrape_maxvision <- function(date_begin, date_end, file_type, test_step, file_name) {
    ...
    if(date_end - date_begin > 10) {
        ...
        for(i in 1:nrow(searchTable)) {
            ...
            if(i == 1) {
                datalogFileList <- searchMaxvision(dateBegin = searchTable$Start[i],
                                                   dateEnd = searchTable$End[i],
                                                   fileName = searchTable$FileName[i],
                                                   fileType = searchTable$FileType[i], 
                                                   step = searchTable$Step[i])
            } else {
                datalogFileList <- bind_rows(
                    datalogFileList,
                    searchMaxvision(dateBegin = searchTable$Start[i],
                                    dateEnd = searchTable$End[i],
                                    fileName = searchTable$FileName[i],
                                    fileType = searchTable$FileType[i], 
                                    step = searchTable$Step[i])
        ...
        datalogFileList <- searchMaxvision(dateBegin = format(date_begin, "%m-%d-%Y"),
                                           dateEnd = format(date_end, "%m-%d-%Y"),
                                           fileName = file_name,
                                           fileType = file_type,
                                           step = test_step)
    }
